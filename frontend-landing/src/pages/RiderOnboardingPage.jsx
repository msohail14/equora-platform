...
import { verifyFirebaseToken, bypassOtp, sendMagicLink, setCredentials } from '../lib/api';
import PhoneInput from '../components/PhoneInput';

const APP_STORE_URL = '#';
const PLAY_STORE_URL = '#';
const STEPS = ['Account', 'Set Password', 'Get the App'];
...
const RiderOnboardingPage = () => {
  ...
  const handleVerifyPhoneOtp = useCallback(async () => {
    ...
    try {
      ...
      if (verificationId === 'bypass') {
        ...
        result = await verifyFirebaseToken({ idToken, role: 'rider', phone });
      }
  ...
  const handleSendMagicLink = useCallback(async () => {
    ...
    setLoading(true);
    try {
      await sendMagicLink({ email, purpose: 'signup', role: 'rider' });
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="mx-auto max-w-2xl px-4 py-4">
          ...
          <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
            <div className="h-full bg-emerald-600 rounded-full transition-all duration-500" style={{ width: `${((step) / (STEPS.length - 1)) * 100}%` }} />
          </div>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-xl px-4 py-12">
        ...
        {step === 0 && !authMethod && (
          <div>
            <h1 className="text-2xl font-bold text-gray-900 mb-2">Join Equora as a Rider</h1>
            <p className="text-gray-600 mb-8">Create your account to start booking training sessions.</p>
            <div className="space-y-4">
              <div className="rounded-xl border border-gray-200 bg-white p-6">
                <h3 className="font-semibold text-gray-900 mb-3">Sign up with Phone</h3>
                <PhoneInput value={phone} onChange={setPhone} disabled={loading} />
                <button onClick={handleSendPhoneOtp} disabled={phone.length < 10 || loading}
              ...
              <div className="rounded-xl border border-gray-200 bg-white p-6">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={handleSendMagicLink} disabled={!email.trim() || loading}
        ...
        {step === 0 && authMethod === 'phone' && (
          <div>
            ...
            <input type="text" maxLength={6} placeholder="123456" value={otp} onChange={(e) => setOtp(e.target.value.replace(/\D/g, ''))}
              className="w-full rounded-lg border border-gray-200 px-4 py-3 text-center text-2xl tracking-[0.5em] font-mono focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 mb-6" />
            <button onClick={handleVerifyPhoneOtp} disabled={otp.length !== 6 || loading}
        ...
        {step === 0 && authMethod === 'email_sent' && (
          <div className="text-center">
            ...
            <button onClick={() => { setAuthMethod(null); setEmail(''); }} className="mt-6 text-sm text-emerald-600 hover:text-emerald-700 font-medium">Use a different method</button>
          </div>
        )}
          ...
          <div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="email" value={credEmail} onChange={(e) => setCredEmail(e.target.value)} placeholder="you@email.com"
              ...
              <div>
                ...
                <input type="password" value={credPassword} onChange={(e) => setCredPassword(e.target.value)} placeholder="At least 6 characters"
                  className="w-full rounded-lg border border-gray-200 px-4 py-3 text-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Confirm Password</label>
                <input type="password" value={credConfirm} onChange={(e) => setCredConfirm(e.target.value)} placeholder="Re-enter password"
                  className="w-full rounded-lg border border-gray-200 px-4 py-3 text-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20" />
              </div>
            </div>
            <div className="mt-8 flex gap-3">
              ...
              <button onClick={() => setStep(2)} className="rounded-lg bg-gray-100 px-6 py-3 text-sm font-medium text-gray-700 hover:bg-gray-200 transition">
                Skip
              </button>
            </div>
          </div>
        ...
        {step === 2 && (
          <div className="text-center py-8">
            ...
            <p className="text-gray-600 mb-8">Download the Equora app to browse coaches, book sessions, and start riding.</p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center mb-8">
              <a href={APP_STORE_URL} className="inline-flex items-center justify-center gap-2 rounded-xl bg-gray-900 px-8 py-4 text-base font-semibold text-white shadow-lg hover:bg-gray-800 transition">
              ...
              <a href={PLAY_STORE_URL} className="inline-flex items-center justify-center gap-2 rounded-xl bg-gray-900 px-8 py-4 text-base font-semibold text-white shadow-lg hover:bg-gray-800 transition">
